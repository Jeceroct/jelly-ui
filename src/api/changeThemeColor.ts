...
export const changeThemeColor = (color: JColor): void => {
  changeColor(color, document.documentElement);
};

export const changeColor = (color: JColor, element: HTMLElement): void => {
  ...
  element.style.setProperty(
    `--theme-disabled`,
    `rgba(
    calc(var(--theme-r) * 0.9),
    calc(var(--theme-g) * 0.9),
    calc(var(--theme-b) * 0.9),
    0.8)`
  );
  element.style.setProperty(
    `--text`,
  ...
  element.style.setProperty(
    `--text-disabled`,
    rgb_r * 0.299 + rgb_g * 0.587 + rgb_b * 0.114 > 130? "black" : "white"
  )
};
